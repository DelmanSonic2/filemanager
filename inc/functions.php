...
while ($file = readdir($dir)) {
    if (is_dir($folder  .'/'. $file) && $file != '.' && $file != '..' && $name = $folder . '/' . $file) {
        $html .= '<li><div class=accordion><img src="/img/folder.svg"><span>' . $file .'</span></div>';
    ...
    if ((getExtension($file) == 'png'||getExtension($file) == 'jpg'||getExtension($file) == 'webp') && $name = $folder . '/' . $file) {
